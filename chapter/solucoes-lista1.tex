...
\quad \text{e}\quad \abs{d(x_1,y_1)-d(x_2,y_2)}\le d(x_1,x_2)+d(y_1,y_2).$$}
...
% exercício2
\begin{exercicio}{Sejam $A,B\subset X$ e $D(A,B)=\inf\{d(x,y);\, x\in A,\ y\in B\}$, $D(x,B)=\inf_{y\in B} d(x,y)$, vejamos que 
  valem as seguintes propriedades:
  \begin{enumerate}[label=(\alph*)]
  \item $D$ não é métrica nas partes de $X$;
  \item $\abs{D(x,B)-D(z,B)}\le d(x,z)$. 
\end{enumerate}}
\textbf{(a)} Falham simultaneamente a separação e a desigualdade triangular. 

\emph{Falha da separação:} Basta considerar $A, B \subset X$ satisfazendo $A \cap B \neq \emptyset$;

\emph{Falha da desigualdade triangular:} Dados $A, B, C \subset X$ satisfazendo
  $D(A, B) > 0$, $A \cap C \neq \emptyset$ e $B \cap C \neq \emptyset$,
temos 
\begin{align*}
  D(A, B) > D(A, C) + D(B, C) = 0
\end{align*}
...
\textbf{(b)} Se $B=\emptyset$ a afirmação é vazia; suponha $B\neq\emptyset$. Dados $x, z \in X$,
    da definição de ínfimo, para todo $\epsilon>0$, 
existe $y \in B$ tal que, 
\begin{align*}
  d(z, y) \le D(z, B) + \frac{\epsilon}{2} \implies -d(z, y) \ge -D(z, B) - \frac{\epsilon}{2};
\end{align*}
  Já que $d(x, y_0) \ge D(x, B)$ para todo $y_0 \in B$, segue da desigualdade acima que,
\begin{align*}
  d(x, z) \ge d(x, y) - d(z, y) \ge D(x, B) - D(z, B) - \frac{\epsilon}{2}, \quad \forall \epsilon>0.
\end{align*}
  Sendo assim, $D(x, B) - D(z, B) \le d(x, z)$. De maneira análoga temos $D(z, B) - D(x, B) \le d(x, z)$, e, por conseguinte 
...
\[ B(x_0,1)=\{x;\ d(x_0,x)<1\}=\{x_0\}, \qquad \overline{B}(x_0,1)=\{x;\ d(x_0,x)\le 1\}=X. \]
  Note que $\{x_0\} = \overline{\set{x_0}}$, pois a única sequência que pode ser tomada em $\set{x_0}$ é a sequência constante. 
Logo $\overline{B(x_0,1)}=\{x_0\}\neq X=\overline{B}(x_0,1)$.

\begin{obs}
Vale sempre a inclusão $\overline{B(x,r)}\subset \overline{B}(x,r)$,
  pois definindo 
\begin{align*}
  f_x: X &\to \bb R^+\\
  y &\mapsto f_x(y) = d(x, y).
\end{align*}
  temos $f_x$ contínua pelo \hyperlink{sol:lista1:1}{exercício 1 da lista 1}. Já que
  $\overline{B}(x,r) = f_x^{-1}([0, r])$, temos que $\overline{B}(x, r)$ é fechado. 
  Mais ainda,
  \begin{align*}
    [0, r) \subset [0, r] \implies B(x, r) = f_x^{-1}([0, r)) \subset f_x^{-1}([0, r]) = \overline B(x, r).
  \end{align*} 
Daqui, basta notar que dado $A \subseteq B$ temos $\overline A \subseteq \overline B$. Por conseguinte, 
  $\overline{B(x, r)} \subset \overline{\overline B(x, r)} = \overline B(x, r)$.
\end{obs}
\end{exercicio}
...
% exercício7
\begin{exercicio}{Dado $X=X_1\times X_2$, vejamos que ele será um espaço métrico quando munido com as seguintes métricas 
\begin{align*}
 d_a(x,y)=\sqrt{d_1(x_1,y_1)^2+d_2(x_2,y_2)^2} \quad \text{,} \quad d_b = d_1+d_2 \quad \text{e} \quad d_c=\max\{d_1,d_2\}.
\end{align*}
}
Em todos os três casos, não-negatividade e simetria são imediatas de $d_1,d_2$ serem métricas, e
\[ d_\bullet(x,y)=0 \iff d_1(x_1,y_1)=d_2(x_2,y_2)=0 \iff x_1=y_1 \text{ e } x_2=y_2 \iff x=y, \]
pois em cada caso $d_\bullet(x,y)=0$ força as duas parcelas a se anularem. 

Resta a desigualdade triangular; sejam $x,y,z\in X$ e ponha $a_i=d_i(x_i,z_i)$, $b_i=d_i(z_i,y_i)$, de modo que $d_i(x_i,y_i)\le a_i+b_i$ para $i=1,2$.
  \vspace{4mm} 

Daqui, vejamos cada um dos casos,\\ 
\emph{$d_b$:} Segue diretamente da desigualdade triangular das métricas 
  $d_1$ e $d_2$.

\emph{$d_c$:} Para $i=1,2$, note que 
  $$d_i(x_i,y_i)\le a_i+b_i \le \max\{a_1,a_2\}+\max\{b_1,b_2\}.$$ 
Tomando o máximo em $i$ segue o resultado.

\emph{$d_a$:} Já que $t\mapsto \sqrt{t_1^2+t_2^2}$ é a norma euclidiana $\norm{\cdot}_2$ de $\R^2$ e é monótona em cada coordenada sobre vetores de coordenadas não-negativas,
\[ d_a(x,y)=\norm{(d_1(x_1,y_1),d_2(x_2,y_2))}_2 \le \norm{(a_1+b_1,\,a_2+b_2)}_2 \le \norm{(a_1,a_2)}_2+\norm{(b_1,b_2)}_2, \]
Ou seja, $d_a(x,y)\le d_a(x,z)+d_a(z,y)$.
...
% exercício10
\begin{exercicio}{Vejamos que se $x\in \ell^1$ e $y\in \ell^\infty$, então 
  $$\sum_{j\ge1}\abs{x_jy_j}\le\norm{x}_1\norm{y}_\infty.$$}
...
\begin{exercicio}{Se $1<p<q<\infty$, então $\ell^1\subset \ell^p\subset \ell^q\subset \ell^\infty$, com inclusões próprias.}
  ...
  \emph{Inclusões.} Sejam $1\le r<s<\infty$ e $x\in \ell^r$. Como $\sum_j\abs{x_j}^r<\infty$, temos $\abs{x_j}\to0$; logo $\abs{x_j}\le 1$ para $j\ge j_0$ e, como para $t \in [0, 1]$ temos $t^\alpha \le t^\beta$ sempre que $\alpha \ge \beta$,
  então $\abs{x_j}^s\le\abs{x_j}^r$. Portanto
  \begin{align*}
    \sum_{j\ge j_0}\abs{x_j}^s\le \sum_{j\ge j_0}\abs{x_j}^r<+ \infty 
    \implies \sum_{j \in \bb N} \abs{x_j}^s = \sum_{j = 1}^{j_0 - 1} \abs{x_j}^s +\sum_{j= j_0}^\infty\abs{x_j}^s < +\infty. 
  \end{align*}
Ou seja, $\ell^1\subset \ell^p\subset \ell^q$. Para $\ell^q\subset \ell^\infty$, basta notar que, dado $x \in \ell^q$, 
\begin{align*}
  |x_k|^q \le \sum_{j \in \bb N} \abs{x_j}^q = \norm{x}_q^q, \quad \quad \forall k \in \bb N.
\end{align*}
  Portanto, $\abs{x_k} \le \norm{x}_q$ para cada $k \in \bb N$, e, por conseguinte, $\norm{x}_\infty \le \norm{x}_q$.
...
\emph{As inclusões são próprias:} para as $p$-normas com $p<+\infty$, isso é obtido forçando-se a aparição da série harmônica.
Para a norma $\ell^\infty$ basta considerar uma sequência constante.
...
% exercício15
\begin{exercicio}{Vejamos que $1\le p<\infty$, temos $\ell^p\subset c_0\subset \ell^\infty$. Além disso, exibiremos um 
  elemento de $c_0$ que não pertence a nenhum $\ell^p$.}
